...
ENV http_proxy=http://10.10.80.26:34512
ENV https_proxy=http://10.10.80.26:34512
...
RUN chown -R user:user /opt/app /input /output
...
# COPY --chown=user:user image_transfer.py create_convert_dict.py output_rename.py ./

COPY --chown=user:user inference.sh ./
...
COPY --chown=user:user smile/ /opt/app/smile/
